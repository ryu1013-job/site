...
**図:** 判断の流れは [FLOW.md](./FLOW.md) にフローチャートとしてまとめてある。
...
| R08 | Stage=S2 または `renewal_due` | A05 | 1 |
| R09 | Stage=S2 | A06 | 2 |
| R10 | Stage=S3 | A07 | 1 |
...
`R08` が更新時にも発火するのは、更新でも認定調査があるためだ。初回と同じ準備が必要になる。
...
A03 と A01 の同時提示を禁じるのは、入院中の相手にとって病院の相談室が正しい入り口だからだ。両方に電話するよう指示するのは、この製品が消そうとしている種類のノイズそのものになる。
...
`(priority asc, alertLinked desc, ruleIndex asc)` で安定ソート。

`alertLinked` は「そのアクションが現在立っているアラート・期限に直接応えるか」。同じ優先度なら、期限を持つアクションを先に出す。更新間近なら A09（更新申請）が A05（困りごとメモ）より先、という順序をここで担保する。ルール配列の順序だけをタイブレークにすると、期限のあるアクションが後ろに回りうる。
